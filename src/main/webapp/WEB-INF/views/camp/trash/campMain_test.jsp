...
<body>
	<a href="/campAdmin/campInsert.kh" style="visibility: hidden;">캠핑장
		정보 삽입</a>
	<div class="container">
		<div class="table-responsive">
			<p align="left" style="font-size: 34px;">
				<a href="/campAdmin/campAdminMain.kh"
					style="color: black; text-decoration: none;">캠핑장 리스트 관리</a>
			</p>
			<table align="center" border="1" width=""
					...
					<td colspan="8" align="center">
						<form action="/campAdmin/campAdminMain.kh" method="get">
							<div align="center">
								...
								<div style="display: inline-block;">
									<select class="selectList form-select" name="city"
										id="city_select">
								...
								<div style="display: inline-block;">
									<select class="selectList form-select" name="category"
										id="category_select">
								...
								<div style="display: inline-block;">
									<input style="width: 300px; height: 33px;" type="text"
										name="search">
								</div>
								<div style="display: inline-block;">
				...
				<c:if test="${!empty cList }">
					<tr>
						<td colspan="8" align="center"><b> [검색갯수 : ${count} ]
								${sList.city} ${sList.category} ${sList.search} 검색결과</b></td>
					</tr>
				</c:if>
				<tr align="center" height="20">
					<td colspan="8"><c:if test="${currentPage != 1 }">
							<a
								href="/campAdmin/campAdminMain.kh?page=0&city=${sList.city}&category=${sList.category}&search=${sList.search}&regist=${sList.regist}"
								class="btn btn-primary">처음</a>
							<a
								href="/campAdmin/campAdminMain.kh?page=${currentPage -2}&city=${sList.city}&category=${sList.category}&search=${sList.search}&regist=${sList.regist}"
							...
							<c:if test="${currentPage ne p }">
								<a
									href="/campAdmin/campAdminMain.kh?page=${p -1 }&city=${sList.city}&category=${sList.category}&search=${sList.search}&regist=${sList.regist}"
									class="btn btn-light">${p }</a>
							</c:if>
						</c:forEach> <c:if test="${maxPage > currentPage }">
							<a
								href="/campAdmin/campAdminMain.kh?page=${currentPage }&city=${sList.city}&category=${sList.category}&search=${sList.search}&regist=${sList.regist}"
								class="btn btn-primary">></a>
							<a
								href="/campAdmin/campAdminMain.kh?page=${maxPage-1}&city=${sList.city}&category=${sList.category}&search=${sList.search}&regist=${sList.regist}"
								class="btn btn-primary">끝</a>
						</c:if></td>
				</tr>
